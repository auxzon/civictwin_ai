...
def draw_monogram(size, transparent=False):
    ...
    draw.arc(
        bbox, start=135, end=405, fill=(241, 243, 245, 24), width=stroke_width + 4
    )  # Subtle shadow glow
    ...
    draw.line(
        [center, y_top, center, center + t_height], fill=(0, 229, 255, 32), width=stroke_width + 4
    )
    ...
    draw.line(
        [center, y_top, center, center + t_height], fill=(0, 229, 255, 255), width=stroke_width
    )
...
def main():
    ...
    draw_monogram(192, transparent=False).save(
        os.path.join(icons_dir, "Icon-maskable-192.png"), "PNG"
    )
    draw_monogram(512, transparent=False).save(
        os.path.join(icons_dir, "Icon-maskable-512.png"), "PNG"
    )
